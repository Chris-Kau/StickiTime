    ...
    return (
        <div className = "flex bg-[#D9D9D9] w-screen h-screen justify-center align-middle items-center p-1">
            <div className = "flex flex-row justify-end gap-3 w-full max-w-[100% - 4px] h-full max-h-[100% - 4px]  bg-[#FFFBF5] p-[2px]">
                ...
                {/* Bookmark */}
                <div onClick = {() => openBookmarks()} className = "flex flex-col justify-center items-center  gap-0 max-h-11 transition-all duration-200 hover:bg-black/20">
                    <BookmarkIcon className = "w-6 h-6 mt-1"/>
                    <button className="-mt-1" >Bookmark</button>
                </div>
                
                {/* Timer */}
                <div onClick = {() => openTimer()} className = "flex flex-col justify-center items-center  gap-0 max-h-11 transition-all duration-200 hover:bg-black/20" >
                    <ClockIcon className = "w-6 h-6 mt-1"/>
                    <button className="-mt-1">Timer</button>
                </div>

                {/* New Note */}
                <div onClick = {() => openStickyNote()} className = "flex flex-col justify-center items-center  gap-0 max-h-11 transition-all duration-200 hover:bg-black/20">
                    <StickyNoteIcon className = "w-6 h-6 mt-1"/>
                    <button className="-mt-1" >New Note</button>
                </div>

                {/* Sticky Notes */}
                <div onClick = {() => openBookmarks()} className = "flex flex-col justify-center items-center  gap-0 max-h-11 transition-all duration-200 hover:bg-black/20">
                    <StickyNotesFolderIcon className = "w-6 h-6 mt-1"/>
                    <button className="-mt-1" >Sticky Notes</button>
                </div>
